...
def index(request: Request):
    ...
    if pairs_filter == "Intraday price high (close)":
        cursor.execute("""
        SELECT * FROM (
            SELECT pair, pair_id, max(close), open_time
            FROM pairs_price
            JOIN currencies on currencies.id = pairs_price.pair_id
            GROUP BY pair_id
            ORDER BY pair
        ) WHERE open_time > ? """, (day_today,))

    if pairs_filter == "Intraday price low (close)":
        cursor.execute("""
        SELECT * FROM (
            SELECT pair, pair_id, min(close), open_time
            FROM pairs_price
            JOIN currencies on currencies.id = pairs_price.pair_id
            GROUP BY pair_id
            ORDER BY pair
